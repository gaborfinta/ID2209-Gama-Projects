 ...
 * 
 * Guests have a 5% chance of being created as zombies (previously isZombie)
...
//	float preferenceStageCrowdSizeBias <- 1.0 + rnd(0.0,1.0);
		 	...
		 	if(!empty(stages))
		 	{
			 	loop i from: 0 to: length(stages)-1
			 	{
			 		...
			 		if(length(myself.stageUtilities) < length(stages))
			 		{
						...
						// Zombies like techno
						if(myself.isZombie and stg.stageGenre = "trashy techno")
						{
							utility <- utility * 100;
						}
	...
	reflex alwaysThirstyAlwaysHungry when: targetAuction = nil
	{
		...
		{	
			...
			// if the guest is not a zombie, they will eat normally
			if(!isZombie)
			{
			...
			//If the guest is a zombie, they will instead seek out another guest and bite them
			else
			{
				float preyDistance <- 200.0;
				Guest targetPrey <- nil;
				loop prey over: Guest
				{
					if(!prey.isZombie and location distance_to(prey.location) < preyDistance)
					{
						preyDistance <- location distance_to(prey.location);
						targetPrey <- prey;
					}
				}
				if(targetPrey != nil)
				{
					write name + " heads for " + target;
					target <- targetPrey;
				}
			}

			// If no valid store was found in the brain, head to info center
			// Even zombies will just eat normally at a kiosk if there are no unzombified guests
			if(target = nil)
			{
...
species Security skills:[moving]
{
	...
	{
		// The guard has a 70% chance of beating the zombie
		// and a 30% chance of becoming a zombie
		if(flip(0.7))
		{
			ask targets[0]
			{
		...
		{
			write name + " was bitten by " + targets[0]; 
			create Guest
			{
				location <- myself.location;
				isZombie <- true;
